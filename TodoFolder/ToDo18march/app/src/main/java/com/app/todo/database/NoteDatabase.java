...
public class NoteDatabase extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        String Todo_Addnote_TABLE = Constants.CREATE_TABLE  + Constants.TABLE_Addnote + "("
                + Constants.Title + Constants.text + Constants.Description + Constants.text + Constants.id+Constants.text+")";
    ...
    public DataModel getNotes(String title) {
        ...
        if (cursor != null)
            cursor.moveToFirst();
        DataModel model = new DataModel(cursor.getString(0), cursor.getString(1),cursor.getString(2));
        return model;
    }
