...
public class TodoHomeInteractor implements TodoHomeInteractorInterface {
   ...
   /* @Override
    public void moveToNotes(TodoHomeDataModel todoHomeDataModel) {
        presenter.showProgressDialog(context.getString(R.string.moving_to_note));

        if(CommonUtils.isNetworkConnected(context)){
            String userId = FirebaseAuth.getInstance().getCurrentUser().getUid();
            databaseReference.child(userId).child(todoHomeDataModel.getStartdate())
                    .child(String.valueOf(todoHomeDataModel.getId()))
                    .child("note_details").setValue(false);
