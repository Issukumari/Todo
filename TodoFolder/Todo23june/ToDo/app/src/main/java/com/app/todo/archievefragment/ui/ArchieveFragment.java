...
public class ArchieveFragment extends Fragment implements ArchieveFragmentInterface, OnSearchTextChange, SearchView.OnQueryTextListener {
    ...
    RecyclerAdapter Archieve_recyclerAdapter;
    boolean isGrid = true;
    ...
    private void initView(View view) {
        ...
        Archieve_recyclerAdapter = new RecyclerAdapter(getActivity());
        archievedRecyclerView.setAdapter(Archieve_recyclerAdapter);
        setHasOptionsMenu(true);
        ...
        ItemTouchHelper.SimpleCallback simpleCallback = new ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.RIGHT | ItemTouchHelper.LEFT) {
            ...
            public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction) {
                ...
                if (direction == ItemTouchHelper.LEFT) {
                    ...
                    databaseReference.child(uId).child(todohomedatamodel.getStartdate()).child(String.valueOf(todohomedatamodel.getId())).setValue(todohomedatamodel);
                    Archieve_recyclerAdapter.deleteditem(position);
                    archievedRecyclerView.setAdapter(Archieve_recyclerAdapter);
                    Snackbar snackbar = Snackbar
                            .make(getActivity().getCurrentFocus(), getString(R.string.move_to_trash), Snackbar.LENGTH_LONG);
                    snackbar.show();
                }
                if (direction == ItemTouchHelper.RIGHT) {
                    try {
                        todohomedatamodel = deleteNoteList.get(position);
                        databaseReference.child(uId).child(todohomedatamodel.getStartdate()).child(String.valueOf(todohomedatamodel.getId())).child("isArchieve").setValue(false);
                        Snackbar snackbar = Snackbar
                                ...
                                .setAction(getString(R.string.UNDO), new View.OnClickListener() {
                                    ...
                                    public void onClick(View view) {
                                        todohomedatamodel.setDeleted(true);
                                        databaseReference.child(uId).child(todohomedatamodel.getStartdate()).child(String.valueOf(todohomedatamodel.getId())).child("isArchieve").setValue(true);
                                    }
                                });
    ...
    public void getNoteListSuccess(List<TodoHomeDataModel> noteList) {
        ...
        deleteNoteList = archievedList;
        datamodels = archievedList;

        Archieve_recyclerAdapter.setList(archievedList);
    }
    ...
    public boolean onQueryTextChange(String newText) {
        ...
        Archieve_recyclerAdapter.setList(noteList);
        return true;
    }
