...
public class ScalingFactor
{
   ...
   // it is possible that the BBox is not occuped completly.
                                                ...
                                                double imgHt, FillLogic fillLogic)
   {
      ...
      XONUtil.debugLog(Log.VERBOSE, "Img aspectRatio: "+aspectRatio+" Disp aspectRatio: "+
                       dispAspectRatio);
   ...
   public String[] getZoomValueList()
   {
      ...
      for(int i = 0; i < m_ZoomValueList.length; i++)
         zoomList[i] = Integer.valueOf(m_ZoomValueList[i]).toString();
      return zoomList;
   }
