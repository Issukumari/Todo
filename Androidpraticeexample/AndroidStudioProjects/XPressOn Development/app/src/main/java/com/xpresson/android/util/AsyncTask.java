...
public abstract class AsyncTask<Params, Progress, Result> 
{
   ...
   // LinkedBlockingQueue orders elements FIFO (first-in-first-out). The head of the 
   // queue is that element that has been on the queue the longest time. The tail of 
   // the queue is that element that has been on the queue the shortest time. 
   ...
   // Here creating ThreadPoolExecutor with the Queue for holding the tasks to be executed 
   // and the ThreadFactory to use when the executor creates a new thread
    ...
    */
   public static final Executor SERIAL_EXECUTOR = XONUtil.hasHoneycomb() ? new SerialExecutor() :
   ...
   // This class executes one task at a time in a serial order.
   @TargetApi(11)
   ...
   private void finish(Result result) 
   {
       ...
       XONUtil.logDebugMesg("Status: "+mStatus);
   }
