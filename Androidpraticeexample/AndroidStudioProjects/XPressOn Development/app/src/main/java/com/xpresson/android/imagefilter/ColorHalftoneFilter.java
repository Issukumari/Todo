...
public class ColorHalftoneFilter {
   ...
   public ColorHalftoneFilter(float dotRadius) { this.dotRadius = dotRadius; }
   ...
   public int[] filter( int[] src ,int w, int h ) {
	   return filter(src ,w ,h ,null , null);
	}

    public int[] filter( int[] src ,int w, int h, int[] outPixels, int[] dst) {
        int width = w;
        int height = h;
        ...
        if (outPixels == null) outPixels = new int[width];
        int[] inPixels = src;
        if (dst == null) dst = new int[w * h];
