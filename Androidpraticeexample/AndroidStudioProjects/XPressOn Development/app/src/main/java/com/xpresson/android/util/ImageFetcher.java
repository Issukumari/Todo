...
public class ImageFetcher extends ImageResizer
{
   ...
   // Creating the DiskLruCache Object that maintains the cache in the file system i.e.
   // the mHttpCacheDir for the Image Retrieved from the URL
   ...
   private void init(Context context) 
   {
       ...
       mHttpCacheDir = XONUtil.getDiskCacheDir(context, HTTP_CACHE_DIR);
   }
   ...
   */
   ...
   private void initHttpDiskCache() 
   {
      ...
      {
         if (XONUtil.getUsableSpace(mHttpCacheDir) > HTTP_CACHE_SIZE) 
         {
            try {
               mHttpDiskCache = DiskLruCache.open(mHttpCacheDir, 1, 1, HTTP_CACHE_SIZE);
               if (BuildConfig.DEBUG) Log.d(TAG, "HTTP cache initialized");
            } catch (IOException e) { mHttpDiskCache = null; }
         }
   ...
   protected void clearCacheInternal() 
   {
      ...
      synchronized (mHttpDiskCacheLock) 
      {
         if (mHttpDiskCache != null && !mHttpDiskCache.isClosed()) 
         {
            ...
            } catch (IOException e) { Log.e(TAG, "clearCacheInternal - " + e); }
            mHttpDiskCache = null;
   ...
   protected void closeCacheInternal() 
   {
      ...
      synchronized (mHttpDiskCacheLock) 
      {
         if (mHttpDiskCache != null) 
         {
            ...
            } catch (IOException e) { Log.e(TAG, "closeCacheInternal - " + e); }
         }
      }
   }
   ...
   private Bitmap processBitmap(String data) 
   {
      ...
      FileInputStream fileInputStream = null; FileDescriptor fileDescriptor = null; 
      ...
      synchronized (mHttpDiskCacheLock) 
      {
         ...
         {
            try { mHttpDiskCacheLock.wait(); } catch (InterruptedException e) {}
         }

         if (mHttpDiskCache == null) return null;
         try {
