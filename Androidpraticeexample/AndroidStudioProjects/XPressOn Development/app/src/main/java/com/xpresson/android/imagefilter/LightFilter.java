...
public class LightFilter extends WholeImageFilter 
{
   ...
   /**
    * Use a separate image alpha channel as the bump map. This is not SUPPORTED
    */
                                ...
                                Rect transformedSpace)
   {
      ...
      Color4f diffuseColor = new Color4f( material.diffuseColor );
      Color4f specularColor = new Color4f( material.specularColor);
      Function2D bump = bumpFunction;
      
      // Apply the bump softness - THE IMAGE SOURCE IS NOT SUPPORTED
      if (bumpSource == BUMPS_FROM_IMAGE || bumpSource == BUMPS_FROM_IMAGE_ALPHA || 
          bumpSource == BUMPS_FROM_MAP || bump == null) 
      {
         if ( bumpSoftness != 0 ) {
            ...
            GaussianFilter.convolveAndTranspose(kernel, bumpPixels, tmpPixels, bumpWidth, 
                           bumpHeight, true, false, false, GaussianFilter.WRAP_EDGES );
            GaussianFilter.convolveAndTranspose( kernel, tmpPixels, softPixels, bumpHeight, 
                           bumpWidth, true, false, false, GaussianFilter.WRAP_EDGES );
            bump = new ImageFunction2D(softPixels, bumpWidth, bumpHeight, 
                       ImageFunction2D.CLAMP, bumpSource == BUMPS_FROM_IMAGE_ALPHA);
...
//                      v = v > 0.5f ? 0.5f : v;
   ...
   protected Color4f phongShade(Vector3f position, Vector3f viewpoint, Vector3f normal, 
                                Color4f diffuseColor, Color4f specularColor, 
